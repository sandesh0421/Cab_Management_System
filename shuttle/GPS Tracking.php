...
<script type="text/javascript">
  ...
  {
    window.alert("Shuttle is on its way!");
  }
...
if($conn->connect_error){
  ...
  $result2= $conn->query ("SELECT * FROM shuttle_details where sid = '$v' ORDER BY sid DESC LIMIT 1");
...
<div class= "panel panel-body detail">
      ...
      <!-- <tr>
         <td align="center">Time for nearest shuttle: <?php echo "$t";?> minute(s)</td>
      </tr>
      <tr>
         <td align="center">Vacancy in the nearest shuttle: <?php echo "$k";?></td>
      </tr> -->
   </table>
   <br>
   <br>
   <div class= "wrapper">
   <button name="wait" type="submit" class="btn btn-success" value="wait" onclick="displaywait()">Waiting</button>
   <button onclick="location.href='http://localhost/shuttle/book.php'" type="button" class="btn btn-success" value="book">
     Call Shuttle</button>
   <p id="show"></p>
   </div>
      </div>
...
/*
  $o=1;
  $v=$GLOBALS['v'];
  $result2= $conn->query ("SELECT * FROM shuttle_details where shuttle_details.sid = '$v' ORDER BY sid DESC LIMIT 1");
  $row2 = $result2->fetch_assoc();
  $sql3="update shuttle_details set stud_wait=stud_wait+'$o' where shuttle_details.sid='$v'";
  $result3=mysqli_query($conn,$sql3);
  $row3 = mysqli_fetch_assoc($result3);
  if (mysqli_num_rows($result3)>0)
    {
